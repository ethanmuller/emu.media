...
function preload() {
  ...
  const imgBUrl = document.querySelector('#imgB').src;

  audioEl.hidden = true;
  myImage = loadImage(imgUrl);
  myImageB = loadImage(imgBUrl);
...
function draw() {
  ...
  // const rockLFO  = Math.sin(t * Math.PI * bpms * rate * 2);

  // draw counter
  stroke('white');
  noFill();
  circle(width - 20, height/2 - 20, 20);
  circle(width - 20, height/2 + 20, 20);
  noStroke();
  fill('#ffbd00');
  circle(width - 20, height/2 + Math.sign(bounceLFO) * 20, 15 + lfo3 * 5);
  ...
  // }
  image(myImageB, width/2 - myImage.width/2, height - myImage.height);
  translate(myImageB.width, myImageB.height*0.83);
  scale(1, 1 + Math.abs(bounceLFO)*0.15);
  rotate(rockLFO*5)
  translate(-myImageB.width, -myImageB.height*0.83);
  image(myImage, width/2 - myImage.width/2 - 8 + rockLFO * 3, height - myImage.height + 5 + 10 + Math.abs(bounceLFO) * -1 * 30 + 20);
